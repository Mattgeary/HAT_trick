...
infection <- function(popn, human, OE, probe, feed, transmission = transmission.default, adult){
		...
		for(i in 1:length(popn[[1]])){
			...
			for(j in 1:length(popn[[1]])){
				infect[[i]][[j]] <-  matrix(c(rep(0, max.age*3)), ncol = 3)
			}
		}
		...
		for(i in 1:length(probe[[1]])){
			for(j in 1:length(probe[[1]])){
				infect[[i]][[j]] <- ((sum(human[[i]][[j]])/sum(human[[i]][[j]][,3]) * (sum(probe["human"][[i]][[j]]) - sum(feed[["human"]][[i]][[j]])* transmission[["probe"]]$human)) + (sum(human[[i]][[j]])/sum(human[[i]][[j]][,3]) * sum(feed[["human"]][[i]][[j]]) * transmission[["probe"]]$human) + (sum(human[[i]][[j]])/sum(OE[[i]][[j]][,3]) * (sum(probe["OE"][[i]][[j]]) - sum(feed[["OE"]][[i]][[j]])* transmission[["probe"]]$OE)) + ((sum(OE[[i]][[j]])/sum(OE[[i]][[j]][,3]) * sum(feed[["OE"]][[i]][[j]]) * transmission[["probe"]]$OE))) 	
			}
		}
	...
	for(i in 1:length(cell.popn)){
 	...
 	return(list(cell.popn = cell.popn, human = human, OE = OE))
}
